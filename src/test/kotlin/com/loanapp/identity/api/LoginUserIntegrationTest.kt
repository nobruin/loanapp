...
class LoginUserIntegrationTest(
    ...
    fun `should login successfully with correct credentials`() {
        ...
        val result = mockMvc.post(LOGIN_URL) {
        ...
        assertTrue(tokenProvider.validateToken(loginResponse.token),
            "The user token must be a valid token")

        assertEquals(VALID_USER_ID, extractedUserId?.value.toString(),
            "The token's userId must match the logged in user")

        assertEquals(VALID_USER_ID, loginResponse.userId, "" +
